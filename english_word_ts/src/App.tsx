    ...
    interface QuestionData {
      ...
      wrong: string;
      context: string;
      context_translation?: string; // 例文の日本語訳を追加（オプショナル）
      source: string;
    }
    ...
    const DEFAULT_DATA: QuestionData[] = [
      {
        ...
        pronunciation: "/ˈhæzərdəs məˈtɪriəl/",
        pos: "Noun",
        meaning: "危険物",
        wrong: "安全物質",
        context: "A license is required to handle hazardous materials.",
        context_translation: "危険物を取り扱うには免状が必要です。",
        source: "法令"
      },
      {
        id: 2,
        word: "Fire Service Act",
        pronunciation: "/faɪər ˈsɜːrvɪs ækt/",
        pos: "Noun",
        meaning: "消防法",
        wrong: "建築基準法",
        context: "The storage of hazardous materials is regulated by the Fire Service Act.",
        context_translation: "危険物の貯蔵は消防法によって規制されています。",
        source: "法令"
      },
      { id: 3, word: "prescriptive", pronunciation: "/prɪˈskrɪptɪv/", pos: "Adjective", meaning: "規範的な、指示する", wrong: "自由放任の", context: "The manual offers prescriptive rules for grammar and punctuation.", source: "Chicago Manual of Style" },
      { id: 4, word: "authoritative", pronunciation: "/əˈθɔːrɪteɪtɪv/", pos: "Adjective", meaning: "権威ある、信頼できる", wrong: "疑わしい", context: "CMOS is considered an authoritative source for American English.", source: "Chicago Manual of Style" },
      { id: 5, word: "plagiarism", pronunciation: "/ˈpleɪdʒərɪzəm/", pos: "Noun", meaning: "盗作、剽窃", wrong: "独創", context: "Strict citation guidelines help prevent plagiarism.", source: "Chicago Manual of Style" },
      { id: 6, word: "bibliography", pronunciation: "/ˌbɪbliˈɒɡrəfi/", pos: "Noun", meaning: "参考文献目録", wrong: "索引", context: "The bibliography lists all sources consulted by the author.", source: "Chicago Manual of Style" },
      { id: 7, word: "manuscript", pronunciation: "/ˈmænjʊskrɪpt/", pos: "Noun", meaning: "原稿", wrong: "既刊本", context: "Authors submit their manuscripts to the publisher for review.", source: "Chicago Manual of Style" }
    ];
    ...
    const BaseWrapper = ({ children, title, className, onReset }: { children: React.ReactNode, title?: string, className?: string, onReset: () => void }) => {
    ...
    const Button = ({ onClick, children, variant = "primary", className = "", disabled = false, icon: Icon, ...props }: ButtonProps) => {
      const baseStyle = "px-4 py-2 text-sm font-bold uppercase tracking-wide transition-colors flex items-center justify-center gap-2 disabled:opacity-50 disabled:cursor-not-allowed rounded-sm select-none";
    ...
    function App() {
      ...
      const handleManualAudioPlay = () => {
        if (currentQuestion) {
          if (sessionMode === 'quiz' && !showFeedback) {
            playAudioSequence(currentQuestion, 'word');
          } else {
      ...
      const startSession = (mode: SessionMode) => {
        ...
        if (isRandomOrder) {
          ...
          const targetId = isNaN(parsedStart) ? 1 : parsedStart;
          
          let startIndex = pool.findIndex(q => q.id >= targetId);
              ...
              <div className="flex flex-col h-full">
                ...
                <div className="bg-[#1b1a19] border-b border-[#383634] p-2 flex justify-between items-center shrink-0">
                  ...
                  <div className="flex gap-2">
                    <Button variant="ghost" onClick={returnToStart} className="h-8 text-xs px-2 sm:px-4">Cancel</Button>
                    <Button onClick={handleImport} icon={Check} className="h-8 text-xs px-2 sm:px-4 shrink-0">
                      Save
                    </Button>
                  </div>
                </div>
                ...
                <div className="bg-[#1b1a19] p-3 sm:p-4 border-t border-[#383634] flex justify-center items-center shrink-0">
          ...
          <div className="h-screen flex bg-[#161512]">
            <BaseWrapper title="Session Complete" className="w-full h-screen flex flex-col" onReset={handleAppReset}>
              <div className="flex-1 flex flex-col items-center justify-center p-4 sm:p-8 space-y-8 text-center overflow-y-auto">
                ...
                <div className="pt-4 sm:pt-8 w-full max-w-xs space-y-3">
                  {sessionMode === 'quiz' && mistakes.length > 0 && (
                    <Button onClick={startReviewSession} variant="blue" className="w-full py-4" icon={RotateCcw}>
          ...
          <BaseWrapper title={`${sessionMode === 'quiz' ? 'Quiz' : 'Reading'} : ${currentIndex + 1} / ${activeQuestions.length}`} className="w-full h-screen flex flex-col" onReset={handleAppReset}>
            ...
            <div className="flex-1 flex flex-col overflow-hidden">
              <div className="flex-1 flex flex-col items-center justify-center p-4 sm:p-8 relative overflow-y-auto">
                ...
                <div className="mb-4 flex items-center gap-3 mt-4 sm:mt-0">
                  ...
                  <button
                    ...
                    className="p-1.5 text-[#8c8c8c] hover:text-[#dbd9d6] hover:bg-[#383634] rounded-sm transition-colors disabled:opacity-50 focus:outline-none"
                    title="Play Audio"
                  >
              ...
              <div className="h-1/2 sm:h-auto sm:min-h-[20rem] bg-[#1b1a19] border-t border-[#383634] relative flex flex-col overflow-y-auto shrink-0">
                {sessionMode === 'reading' ? (
                  <div className="flex-1 flex flex-col p-4 sm:p-6 h-full min-h-[14rem] sm:min-h-[16rem]">
                      ...
                      {currentQuestion.context && (
                        <div className="text-[#dbd9d6] italic text-sm sm:text-base max-w-lg mt-2 sm:mt-4 px-4 border-l-4 border-[#383634] text-left">
                          ...
                          {currentQuestion.context_translation && (
                            <div className="text-xs sm:text-sm text-[#8c8c8c] not-italic mt-1">
                              {currentQuestion.context_translation}
                            </div>
                          )}
                        </div>
                      ...
                      <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 sm:gap-4 p-4 sm:p-6 h-full min-h-[14rem] sm:min-h-[16rem]">
                        {shuffledChoices.map((choice, idx) => (
                          <button
                            ...
                            className="bg-[#262421] border border-[#383634] hover:bg-[#383634] hover:border-[#8c8c8c] focus:outline-none text-lg sm:text-xl font-bold text-[#dbd9d6] rounded-sm transition-colors active:bg-[#454341] flex items-center justify-center p-3 sm:p-4"
                          >
                            {choice.text}
                          </button>
                        ))}
                      ...
                      <div className="flex-1 flex flex-col p-4 sm:p-6 h-full min-h-[14rem] sm:min-h-[16rem]">
                        ...
                        <div className="mt-4 sm:mt-8 pb-2 shrink-0">
                          <Button 
                            ...
                            className="w-full py-3 sm:py-4 text-base sm:text-lg shrink-0"
                          >
                            Next
                          </Button>
                        </div>
